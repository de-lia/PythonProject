class Node:  # class for the parent node
    ...
    # Preorder traversal: from the root to the left subtree to the right subtree
    def pre_order(self, root):
        my_lis = []
        if root:
            my_lis.append(root.data)
            my_lis = my_lis + self.pre_order(root.left)
            my_lis = my_lis + self.pre_order(root.right)
            return my_lis
